...
****************
// Build plotfile cache && shared cache count

    MultiFab plot(ba, dm, Ncomp, Nghost);
    bool pltDone = false;
    ...
    for (int n = 1; n <= nsteps; ++n)
    {
        if (thread_id == 0)
        {
            ...
            if (plot_int > 0 && n%plot_int == 0)
            {
                ...
                {
                  ****************
                      New multifab in cache[cache+1] (atomic)
                      Pull from GPU into new Multifab
                      cache++;
                  ****************
                }
                if (thread_id == 1)
